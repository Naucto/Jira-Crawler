...
SV_INSTALL_PATH="/opt/naucto-jira-crawler"
...
SV_SERVICE_USER="nrc"
SV_SERVICE_NAME="naucto-jira-crawler"
...
The service requires the use of SSL certificates so that Jira can contact it
...
To access the Jira instance, we will need a URL to it. This URL can point to
an Atlassian cloud instance, or a more classical self-hosted instance.
...
EOF
    ...
    jira_instance_url="`sv_question "What is the URL to your Jira instance?" \
                        "https://naucto.atlassian.net"`"

    if [ -z "$jira_instance_url" ]; then
        echo "$0: No Jira instance URL was specified, cannot continue." >&2
        exit 1
    fi
...
In addition to the URL that you've just specified, we will also need a secret
to access the Jira instance's projects. This secret is called an API token, and
it can be generated from your user profile on the Jira instance OR the
administration panel of your self-hosted instance.
...
EOF
    ...
    if [ -z "$jira_api_token" ]; then
        echo "$0: No Jira API token was specified, cannot continue." >&2
        exit 1
    fi
...
We now need the ID of your Jira project. This is a very short string, usually
3 to 5 characters long, that identifies your project and its tasks.

For example, Naucto's Jira project ID is NCTO.

You can find your project ID by going to your project in Jira and looking
...
EOF
    ...
    if [ -z "$jira_project_id" ]; then
        echo "$0: No Jira project ID was specified, cannot continue." >&2
        exit 1
    fi
...
If you have no project yet, please create one now on your student's repository.
The name of that project is what you need to specify for this question.
